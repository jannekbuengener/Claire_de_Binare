...
class Test72HourOrchestrator:
    ...
    def run_test(self, duration_hours: float = 72.0):
        ...
        self.logger.info(
            f"Starting 72-hour paper trading test (duration: {duration_hours}h)"
        )
    ...
    def _test_iteration(self):
        ...
        breaker_result = self.circuit_breaker.check_breakers(
            {"drawdown": metrics.max_drawdown, "error_rate": 0.01}
        )

        if breaker_result["triggered"]:
            self.logger.warning(
                f"Circuit breakers triggered: {breaker_result['reasons']}"
            )
            self.test_active = False
    ...
    def _generate_test_results(self):
        ...
        return {
            "test_completed": True,
            "duration_hours": (datetime.utcnow() - self.start_time).total_seconds()
            / 3600,
...
def main():
    ...
    parser.add_argument(
        "--duration", type=float, default=72.0, help="Test duration in hours"
    )
