...
def write_report_artifacts(report: dict[str, Any], out_dir: str) -> None:
    ...
    artifact_payload = json.dumps(artifact_report, sort_keys=True, separators=(",", ":"))
    artifact_digest = hashlib.sha256(artifact_payload.encode("utf-8")).hexdigest()
    persisted_report = {
        "schema": "governance.core_secrets_integrity_artifact_digest.v1",
        "artifact_digest_sha256": artifact_digest,
        "redaction_mode": "summary_only",
        "entries_redacted": artifact_report["entries_redacted"],
    }
        ...
        handle.write(
            "\n".join(
                [
                    ...
                    f"**Entries redacted:** `{persisted_report['entries_redacted']}`",
                    f"**Artifact digest (SHA-256):** `{artifact_digest}`",
                    "",
                    "---",
                    "*Generated by `core_secrets_integrity_report.py`*",
                    "",
