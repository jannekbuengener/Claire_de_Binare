...
It can:
- collect canonical docs files from the Claire de Binare repository
...
ROOT_INDEX_FILES = ("docs/meta/REPOSITORY_CANON.md",)
...
def detect_claire_de_binare_repository_root() -> Path:
...
def is_valid_repository(path: Path) -> bool:
    return (
        path.exists()
        and (path / "docs" / "meta" / "REPOSITORY_CANON.md").is_file()
        and (path / "knowledge").is_dir()
        and (path / "agents").is_dir()
    )


def resolve_repository_path(explicit_path: str | None = None) -> Path:
    if explicit_path:
        path = Path(explicit_path).resolve()
        if is_valid_repository(path):
            return path
        raise FileNotFoundError(f"invalid Claire de Binare repository path: {explicit_path}")

    claire_de_binare_repository = detect_claire_de_binare_repository_root()
    if is_valid_repository(claire_de_binare_repository):
        return claire_de_binare_repository.resolve()
    raise FileNotFoundError("could not locate the Claire de Binare repository")


def iter_canonical_docs_files(
    repository_path: Path, include_dirs: Sequence[str] = DEFAULT_INCLUDE_DIRS
) -> list[Path]:
    files: list[Path] = []
    seen: set[Path] = set()
    ...
    for relative_dir in include_dirs:
        base = repository_path / relative_dir
        if not base.is_dir() and relative_dir == "meta":
            base = repository_path / "docs" / "meta"
        if not base.is_dir():
            continue
...
def build_chunks(
    ...
    for file_path in iter_canonical_docs_files(repository_path, include_dirs):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--repository", help="Explicit Claire de Binare repository path")
    parser.add_argument(
        "--include-dir",
        action="append",
        dest="include_dirs",
