...
def health():
    ...
    return jsonify(status="ok" if age < DATA_STALL_SEC*2 else "stale", age_sec=age, symbols=len(klines))
...
def on_open(ws, chunk):
    for s in chunk:
        sub = {"op":"sub","channel":"sub.kline","symbol":s,"interval":KL_INTERVAL}
        ws.send(json.dumps(sub))
        time.sleep(0.005)

def make_ws(chunk):
    return WebSocketApp(WS_URL,
...
def compute_top5():
    ...
    return (df.sort_values("pct_change", ascending=False).head(5),
