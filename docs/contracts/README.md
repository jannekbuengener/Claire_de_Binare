...
| Agent Registry | `cdb_agent_registry.v1.schema.json`, [`agent_registry/CDB_AGENT_REGISTRY_V1.md`](agent_registry/CDB_AGENT_REGISTRY_V1.md), examples under `examples/agent_registry/`, Desired State under `config/agent-control/` | Provider-neutral declarative registry + dry-run reconciler (`#4252`); CLI `python -m tools.agent_control registry …` |
...
| PR Approval Context | `cdb_pr_approval_context.v1.schema.json`, [`agent_approval/CDB_PR_APPROVAL_CONTEXT_V1.md`](agent_approval/CDB_PR_APPROVAL_CONTEXT_V1.md), fixtures under `tests/fixtures/agent_control/approval/` | Repo-backed approval recommendation (`#4257`); CLI `python -m tools.agent_control approval context|drift` |
| Agent Control Pilot | `cdb_agent_control_pilot_report.v1.schema.json`, `cdb_agent_control_pilot_manifest.v1.schema.json`, [`agent_pilot/CDB_AGENT_CONTROL_PILOT_REPORT_V1.md`](agent_pilot/CDB_AGENT_CONTROL_PILOT_REPORT_V1.md), fixtures under `tests/fixtures/agent_control/pilot/` | Mock-first ACP E2E foundation pilot (`#4258` Refs); CLI `python -m tools.agent_control pilot run|verify` |
...
| Research Validation (Wave 3 / Pilot Spec) | `cdb_research_validation_pilot.v1.schema.json` | SPECIFICATION_ONLY three-candidate pilot (#4272); shared contract/gate path, economics stress scenarios, expected artifact slots; no executed evidence — see [`CDB_RESEARCH_VALIDATION_PILOT_SPEC_V1.md`](../research/CDB_RESEARCH_VALIDATION_PILOT_SPEC_V1.md) |
