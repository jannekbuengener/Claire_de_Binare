...
Last reconciliation: 2026-04-19 (LR-012 / #781 issue-state reconciliation, issue #1765; prior: 2026-04-07 LR-050 / #792 and LR-011 / #780 tracker-state reconciliation, issue #1481; 2026-04-05 P4 PASS + P5 prestart GO + #1423 handoff)
...
- `P2` (E2E + Replay) is `DONE`. `P1` is `PARTIAL`. `P3` is `PARTIAL`: `LR-031` comparison is PASS-evidenced; `LR-030` is not closure-ready. Repo-backed are the zero-execution / live-data hard gate in `docs/evidence/LR-030.md` and the committed lean continuity proof in `reports/p5_canary/2026-04-04/lean_shadow_evidence_handoff.yaml` (`soak_minutes: 5`, gate PASS, live data true, zero execution true). Residual uncertainty remains only because the original `LR-030` issue wording still lacks direct committed proof for `>24h` stable shadow mode and functioning alerts. `P4` is `DONE` (`LR-040` PASS 72.19h soak; `LR-041`/`LR-042` closed with evidence). `P5` prestart pack is committed with GO status and a committed lean shadow evidence handoff exists; `LR-050` remains `NO-GO` / fail-closed for live capital. System remains not ready for go-live.
...
| P1 Deterministic Tests | `false` | `LR-010`, `LR-011`, `LR-012` | `PARTIAL` | `LR-010` PASS: [LR-010-EVIDENCE.md](./LR-010-EVIDENCE.md), CI run `23295248170` (2026-03-19); [LR-011 #780](https://github.com/jannekbuengener/Claire_de_Binare/issues/780) closed (GitHub; PR #1106); `LR-012` remains `PARTIAL`: the execution order negative-payload path is repo-backed (PR #1107; fail-closed deterministic no-op with warning + counter and no executor/publish/DB side-effects), but no dedicated `LR-012` evidence file or canonical closure exists for the broader original scope (`candles` / `signals` / visibility) |
...
| P3 Shadow Mode | `false` | `LR-030`, `LR-031` | `PARTIAL` | `LR-030` issue [#784](https://github.com/jannekbuengener/Claire_de_Binare/issues/784) is closed, but not closure-ready. Repo-backed are the zero-execution / live-data hard gate in `docs/evidence/LR-030.md` and the committed lean continuity proof in `reports/p5_canary/2026-04-04/lean_shadow_evidence_handoff.yaml` (`soak_minutes: 5`, gate PASS, live data true, zero execution true). `LR-031` issue [#785](https://github.com/jannekbuengener/Claire_de_Binare/issues/785) is closed; comparison layer is PASS-evidenced in `docs/evidence/LR-031.md` with calibrated thresholds in `docs/evidence/lr031_baseline_thresholds.json`. **Restunsicherheit:** no direct committed proof currently closes the two original residual criteria from `#784`: `>24h` stable shadow mode and functioning alerts. |
...
- P1 is `PARTIAL`: `LR-010` PASS evidenced (CI run `23295248170`, 2026-03-19); `LR-011` closed (GitHub, PR #1106); `LR-012` is not closure-ready. Repo-backed is only the execution order negative-payload path (PR #1107: fail-closed deterministic no-op with warning + counter and no executor/publish/DB side-effects, covered by a green negative unit suite). The broader original scope (`candles` / `signals` / visibility), a dedicated `LR-012` evidence file, and a canonical full-scope closure remain absent.
- P2 is `DONE`: `LR-020-STATE.yaml` = DONE (commit `8c75697`); `LR-021` closed with evidence slices.
- P3 is no longer evidence-empty: `LR-031` is PASS-evidenced and `LR-030` zero-execution behavior is repo-backed plus re-confirmed by the committed lean handoff. Operational status remains `PARTIAL` because the original `LR-030` issue wording still lacks direct committed proof for `>24h` stable shadow mode and functioning alerts.
...
LR reference map (quick reference; issue state reconciled):
