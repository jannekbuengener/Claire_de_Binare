...
| **Max Daily Loss** | `TBD_BLOCKER_BEFORE_LIVE` | **Wired live path:** `RiskManager.check_drawdown_limit()` + `MAX_DAILY_DRAWDOWN_PCT` in [`services/risk/config.py`](../../services/risk/config.py) (generic default **0.05** fraction — *not* canary approval); `decide_trade` threshold derived as percentage points via `risk_config_fraction_to_decision_pct_points` (#4152). **Not wired in order flow:** standalone `CircuitBreaker.check_breakers()` / `LOSS_LIMIT` in [`services/risk/circuit_breakers.py`](../../services/risk/circuit_breakers.py) (class + unit tests only) | `enforceable_now` (drawdown latch path); `blocker_before_live` (canary USDT/%) | Daily PnL below drawdown cap → **BLOCK** + `circuit_breaker_active` via `check_drawdown_limit()` | `tests/unit/risk/test_service.py`; `tests/unit/risk/test_decision_threshold_units.py`; [#2533](https://github.com/jannekbuengener/Claire_de_Binare/issues/2533) |
| **Max Gesamt-Exposure** | `TBD_BLOCKER_BEFORE_LIVE` | [`services/risk/config.py`](../../services/risk/config.py) `MAX_TOTAL_EXPOSURE_PCT` / `MAX_EXPOSURE_PCT` (generic default **0.30** fraction — *not* canary approval); runtime: `check_exposure_limit()` + projected hard gate; `decide_trade` RC_021 uses `fraction * 100` percentage points (#4152). Projected symbol position: `check_projected_position_limit` | `enforceable_now` (mechanism); `blocker_before_live` (canary cap) | Projected exposure/position at/over limit → **BLOCK**; unknown price → **BLOCK** | `tests/unit/risk/test_projected_position_exposure.py`; [#2533](https://github.com/jannekbuengener/Claire_de_Binare/issues/2533) |
...
| **Stop-Loss** | Metadata only | `STOP_LOSS_PCT` on order artifact; protection status `UNAVAILABLE` via evidence ledger in [`core/safety/stop_loss_protection.py`](../../core/safety/stop_loss_protection.py) (#4152, #4186). Trigger + restart-safe consumer + persistent dedup state exist in [`core/safety/stop_loss/`](../../core/safety/stop_loss/) and produce exactly one reduce-only **exit intent** ([`SAFETY_STOP_LOSS_CONSUMER_CONTRACT_V1.md`](../contracts/SAFETY_STOP_LOSS_CONSUMER_CONTRACT_V1.md)). **Open proofs:** real-stack persistence, productive exit path | `docs_only` until real-stack persistence + productive exit path are proven | Cannot claim protection PASS from metadata or from consumer code alone; missing/corrupt/unknown dedup state and unproven intent delivery **BLOCK** | `tests/unit/safety/test_stop_loss_protection.py`; `tests/unit/safety/stop_loss/`; `tests/integration/test_stop_loss_shadow_e2e.py`; evidence `docs/evidence/risk/4152_fail_closed_safety_slice.md`, `docs/evidence/risk/4186_stop_loss_consumer_dedup.md` |
...
| **Kill Switch** | ACTIVE = global halt | [`core/safety/kill_switch.py`](../../core/safety/kill_switch.py); SSOT [`LR-050-KILL-SWITCH-RUNBOOK.md`](./LR-050-KILL-SWITCH-RUNBOOK.md); open-order cancel contract [`EXECUTION_KILL_CANCEL_CONTRACT_V1.md`](../contracts/EXECUTION_KILL_CANCEL_CONTRACT_V1.md) (#4185); P5 precheck `kill_switch_precheck_inactive` in `infrastructure/scripts/soak_gate_eval.py` (precheck gap — runbook §9) | `enforceable_now` | `get_kill_switch_state()` true → **no new orders**; kill-cancel reconciles open synthetic orders (residuals HOLD); fail-closed if state unevaluable (Risk + Execution gates) | `GET /kill-switch`; runbook §6/§9; evidence `docs/evidence/risk/4185_kill_cancel_open_orders.md`; dry-run #2533 |
| **Alertmanager / Prometheus outage** | Policy: fail-closed (no new risk-on) | [`LR-050-OBSERVABILITY-GATES.md`](./LR-050-OBSERVABILITY-GATES.md) §8; P5 governance abort: observability unevaluable per [`config/live-readiness/p5_canary_readiness.yaml`](../../config/live-readiness/p5_canary_readiness.yaml) | `docs_only` | If monitoring cannot attest health: **halt new live orders**, operator manual stop, keep **NO-GO** | [LR-050-OBSERVABILITY-GATES.md](./LR-050-OBSERVABILITY-GATES.md) |
