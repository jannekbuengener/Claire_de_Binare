...
| **Stop-Loss** | Metadata only | `STOP_LOSS_PCT` on order artifact; protection status `UNAVAILABLE` / `ARTIFACT_ONLY` via [`core/safety/stop_loss_protection.py`](../../core/safety/stop_loss_protection.py) (#4152). **No** proven execution consumer/exit path | `docs_only` until E2E consumer | Cannot claim protection PASS from metadata alone | `tests/unit/safety/test_stop_loss_protection.py`; evidence `docs/evidence/risk/4152_fail_closed_safety_slice.md` |
