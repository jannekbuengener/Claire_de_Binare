...
| [#2529](https://github.com/jannekbuengener/Claire_de_Binare/issues/2529) | Kill-switch / manual stop verification SSOT |
...
| **Max Daily Loss** | `TBD_BLOCKER_BEFORE_LIVE` | [`services/risk/config.py`](../../services/risk/config.py) `MAX_DAILY_DRAWDOWN_PCT` (generic default **0.05** — *not* canary approval); circuit `LOSS_LIMIT` **5%** in [`services/risk/circuit_breakers.py`](../../services/risk/circuit_breakers.py); decision threshold `daily_drawdown_pct_max` **5.0** in [`services/risk/service.py`](../../services/risk/service.py) `DECISION_THRESHOLDS` | `enforceable_now` (mechanism); `blocker_before_live` (canary USDT/%) | Drawdown/loss over threshold → **BLOCK** + `circuit_breaker_active`; kill-switch reason `CIRCUIT_BREAKER` in [`core/safety/kill_switch.py`](../../core/safety/kill_switch.py) | `tests/unit/risk/test_service.py`; playbook [`07_RISK_GUARDS_DRAWDOWN_BREAKER.md`](../../knowledge/playbooks/07_RISK_GUARDS_DRAWDOWN_BREAKER.md); [#2533](https://github.com/jannekbuengener/Claire_de_Binare/issues/2533) |
...
| **Cooldown rules** | Active `cooldown_until` (per allocation) | [`services/risk/service.py`](../../services/risk/service.py) + tests in `tests/unit/risk/test_service.py` (cooldown blocks with reason containing `cooldown`) | `enforceable_now` | Active cooldown → **BLOCK** | `tests/unit/risk/test_service.py`; [#2533](https://github.com/jannekbuengener/Claire_de_Binare/issues/2533) |
| **Manual stop rules** | Operator halt (no numeric cap) | [`core/safety/kill_switch.py`](../../core/safety/kill_switch.py) `KillSwitchReason.MANUAL`; runbook [`KILL_SWITCH_OPERATOR_CHECKLIST.md`](../operations/KILL_SWITCH_OPERATOR_CHECKLIST.md); verification SSOT [#2529](https://github.com/jannekbuengener/Claire_de_Binare/issues/2529) | `enforceable_now` | Kill-switch **ACTIVE** → all trading **halt**; cannot bypass | Runbook drill; [#2529](https://github.com/jannekbuengener/Claire_de_Binare/issues/2529) |
| **Circuit Breaker** | Canary thresholds `TBD_BLOCKER_BEFORE_LIVE` (repo defaults documented) | [`services/risk/circuit_breakers.py`](../../services/risk/circuit_breakers.py): drawdown **15%**, loss **5%**, error rate **10%**, frequency **60/min**; `E2E_DISABLE_CIRCUIT_BREAKER` disables for tests; `risk_state.circuit_breaker_active` in [`services/risk/service.py`](../../services/risk/service.py) | `enforceable_now` (mechanism); `blocker_before_live` (canary tuning) | Breaker triggered → **BLOCK** + latch; metric `circuit_breaker_active` | `tests/unit/risk/test_service.py`; [#2529](https://github.com/jannekbuengener/Claire_de_Binare/issues/2529), [#2533](https://github.com/jannekbuengener/Claire_de_Binare/issues/2533) |
| **Kill Switch** | ACTIVE = global halt | [`core/safety/kill_switch.py`](../../core/safety/kill_switch.py); P5 precheck `kill_switch_precheck_inactive` in `infrastructure/scripts/soak_gate_eval.py` | `enforceable_now` | `get_kill_switch_state()` true → **no new orders**; fail-closed if state unevaluable (P5 abort condition) | Endpoint `kill_switch_status`; [#2529](https://github.com/jannekbuengener/Claire_de_Binare/issues/2529) |
| **Alertmanager / Prometheus outage** | Policy: fail-closed (no new risk-on) | Detailed abort/investigate matrix → [#2531](https://github.com/jannekbuengener/Claire_de_Binare/issues/2531); P5 governance abort: observability unevaluable per [`governance/p5_canary_readiness.yaml`](../../governance/p5_canary_readiness.yaml) | `docs_only` | If monitoring cannot attest health: **halt new live orders**, operator manual stop, keep **NO-GO** | [#2531](https://github.com/jannekbuengener/Claire_de_Binare/issues/2531) |
