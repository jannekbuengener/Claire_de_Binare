TL;DR: Hier ist **alles aus deinem Paste** sauber abgeschrieben und nach **Dringlichkeit** sortiert (von “blockt sofort” → “kann warten”). Wenn in den 95 offenen Issues noch mehr steckt, ist es **nicht** in diesem Dump enthalten.
...
* #500 Follow-up: Docker setup — Scout/Trivy/Metrics + Dockerfile strategy (prio:must, type:security)
* #499 Review: Docker setup + CI workflow claims (prio:must, type:security)
...
* #224 E1–E4 Integration: Regime & Allocation Services, strategy_id Pipeline, Risk-Off Enforcement (prio:should)
...
* #497 ci(docker): advanced docker workflow + optimization toolkit
