...
| `sensitivity_campaign_state.py` | Atomic evidence ledger + resume |
...
| `sensitivity_campaign_executor.py` | `FakeExecutor` / refusing real executor |
...
Required bindings include (non-exhaustive):
...
- `resource_budget` (all required fields; hard caps enforced)
- `granted_capabilities = ["campaign_execution"]` only
...
- `execute` requires a live-verified Owner-GO, matching surface + budget, and an
  injected executor. Production CLI uses `RefusingRealExecutor` (no real replays
...
- Real campaign execution against market data in this slice
...
Unit tests use `FakeExecutor`, `tmp_path`, and mocked comment fetchers.
No live GitHub GO comments and no real campaigns.
