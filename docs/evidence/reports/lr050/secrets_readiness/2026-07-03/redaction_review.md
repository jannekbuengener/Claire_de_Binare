...
| C — Evidence commit | Agent + operator attestation | Files under `docs/evidence/reports/lr050/secrets_readiness/2026-07-03/` |
...
```text
rg -i "password|token|api_key|secret|dsn|smtp|mexc|binance|private|credential|@.*\\.|account|AKIA|ghp_|xoxb-|BEGIN .*PRIVATE KEY|[0-9]{1,3}(\\.[0-9]{1,3}){3}" docs/evidence/reports/lr050/secrets_readiness/2026-07-03/
git diff --check
```
