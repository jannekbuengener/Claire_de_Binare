...
- `security-alert-readout.yml` (PR #2495, gemergt 2026-05-15T11:52:26Z, Commit `87827c00`): `feat(security): add alert readout issue automation` — neuer Job `issue-automation` hinzugefuegt; laeuft nach `security-readout`, erzeugt maximal ein GitHub-Issue pro neuer hochgradiger Alert-Gruppe (severity: critical/high/error). Dedupe fail-closed ueber Kommentar-Marker `<!-- cdb-security-new-group:<hash> -->`; GraphQL-Lookup-Fehler fuehren zu skip + exit 2 (nie fail-open). Dry-run ist der sichere Standard; expliziter Live-Modus nur per `workflow_dispatch` mit `issue_automation_live=true`; Scheduled Runs immer Dry-run, unabhaengig vom Dispatch-Input. Permissions job-scoped: `contents: read`, `actions: read`, `issues: write` ausschliesslich im `issue-automation`-Job; Top-Level-Permissions unveraendert. `comment-epic`-Job jetzt `needs: [security-readout, issue-automation]`; Ledger-Boundary-Zeile dynamisch (Dry-run / Live / Skipped + Exit-Code). Kein Auto-Close, keine Alert-Dismissals, kein Auto-Merge; Secret Scanning bleibt redacted/status-only; `comparison_skipped`-Sources ausgeschlossen. `TRIAGE_RUNBOOK.md` §10 (Issue-Automation-Vertrag, Invarianten, Dedupe-Mechanismus, Exit-Codes, Stop-Regeln) ergaenzt. Kein LR-/Live-/Echtgeld-Signal. Epic: #2289.
