...
- Action-Dependency-Bump (PR #1958, 2026-04-27): `aquasecurity/trivy-action` `876cf04`→`ed142fd` (Trivy v0.70.0) in `ci.yaml`, `security-scan.yml`, `trivy.yml` (historischer Stand; `ci.yaml` wurde am 2026-07-16 entfernt). Keine Aenderung an SARIF-Kategorien, Security-Triage-Postur oder operative Control-Logik.
...
- `repository-canon-guard.yml` (PR #1968, 2026-04-27): Der Runtime-Artefakt-Guard erlaubt `docs/runbooks/evidence/*.log` explizit als runbook-nahe Evidence-Ausnahme. Generische `*.log`-Dateien ausserhalb dieses Pfads sowie `logs/`, `__pycache__/`, `.pytest_cache/`, `*.pyc` und `*.pyo` bleiben blockiert. Keine Workflow-/Runtime-/Trading-/LR-Semantik ableiten.
...
- NOISE-FREEZE restore (PR #3662, gemergt 2026-07-01T22:36:27Z, Commit `5f2f1af5`, Issue #3659 / Kampagne #3654): Push- und PR-Trigger fuer 8 security-relevante Workflows nach Billing-Recovery reaktiviert (`codeql-python.yml`, `contracts.yml`, `core-guard.yml`, `docs-conflict-guard.yml`, `repository-canon-guard.yml`, `gitleaks.yml`, `security-scan.yml`, `trivy.yml`). Historische NOISE-FREEZE-Kommentare (#2613) bleiben mit `RESTORED (#3659)` markiert. Kein LR-/Live-/Echtgeld-Signal.
...
- `repository-canon-guard.yml` / artifact hygiene (PR #3668, gemergt 2026-07-01T23:07:48Z, Commit `0eae84ac`, Issue #3667): Vier tracked generierte `artifacts/**/audit.log` entfernt; `.gitignore` gehaertet; Guard auf `main` gruen. Kein LR-/Live-/Echtgeld-Signal.
...
- Workflow-Hygiene (2026-07-16): 13 operativ ungenutzte, geparkte, historische oder intern unerreichbare Workflow-Dateien wurden samt exklusiven Gemini-Commands und workflow-spezifischen Alt-Tests entfernt. Der aktuelle Bestand umfasst 57 dokumentierte Workflows (48 aktiv, 9 manual-only). Kanonischer CI-Pfad bleibt `ci.yml`; Board-Konsistenz läuft über Upsert/Reconcile/Status-Sync. Historische Einträge oberhalb bleiben unverändert und sind keine aktuelle Betriebsanweisung. Kein LR-/Live-/Echtgeld-Signal.
