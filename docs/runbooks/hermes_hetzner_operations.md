...
Explicit `ExecStartPre` mkdir for `/run/hermes/cdb-engineer` (no systemd
RuntimeDirectory / no PrivateTmp) so `ExecStartPost` chown to
`hermes-cdb-engineer` sticks on the host path. Token file remains until explicit
