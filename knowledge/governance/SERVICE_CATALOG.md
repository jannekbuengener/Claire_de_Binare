...
| **Signal** | cdb_signal | 8005 (Runtime) | services/signal/ | [README](../../services/signal/README.md) | **AKTIV** | Signal Generation (`primary_breakout_v1` default nutzt zeitbasierte Lookback-Semantik; `donchian_breakout_v1` nutzt bar-basierte Donchian-Kanaele per Pack-A §7.2; `momentum_builtin` statische Adapter-Grenze); audit metadata: `config_snapshot` (deterministic runtime params snapshot) + `config_hash` (full SHA-256); `SIGNAL_BOT_ID` environment variable wired via compose.red.yml (PR #2129) liefert Experiment-Kontext im SIGNAL-Payload, aber keine neue first-class Ledger-ID; `source=stimulus_fixture` + `stimulus_run_id` erzeugen deterministische `signal_id`s fuer exportierbare paper chains; reserved metadata keys (strategy_id, bot_id, config_snapshot, config_hash, signal_reason, signal_inputs) sind immutable und können nicht durch Candidate-Signal-Metadata überschrieben werden; campaign overlay `manifests/runtime_np_parallel_signal_compose_override.yml` (PR #3918) ergaenzt `cdb_signal_pb1` (Port `8015`, `np-pb1-parallel-01`) + `cdb_signal_donchian` (Port `8016`, `np-donchian-parallel-01`) — nicht kanonischer Default, separate RUNTIME-GO, #3912 NOT READY |
