...
| **Allocation** | cdb_allocation | 8006 | services/allocation/ | [README](../../services/allocation/README.md) | **AKTIV** | Regime→Allocation Mapping; `ALLOCATION_RULES_JSON` in `compose.blue.yml` deckt `paper`, `primary_breakout_v1`, und `donchian_breakout_v1` ab (Donchian nutzt konservative half-cap Regime-Map; PR #3915); campaign overlay `config/arvp/runtime_np_parallel_allocation_compose_override.yml` spiegelt dieselbe Semantik fuer HYP-NP-PARALLEL-2S-01 |
| **Risk** | cdb_risk | 8002 | services/risk/ | [README](../../services/risk/README.md) | **AKTIV** | Risk Gate, Circuit Breaker, Kill-Switch; DECISION ledger writes via `build_decision_ledger_payload()` with `campaign_id` + block-reason context (PR #3961); `/kill-switch` fail-closed; `RealBalanceFetcher` spot REST default `MEXC_BASE_URL=https://api.mexc.com` (PR #3720; no MEXC spot testnet) |
...
| **Signal** | cdb_signal | 8005 (Runtime) | services/signal/ | [README](../../services/signal/README.md) | **AKTIV** | Signal Generation (`primary_breakout_v1` default nutzt zeitbasierte Lookback-Semantik; `donchian_breakout_v1` nutzt bar-basierte Donchian-Kanaele per Pack-A §7.2; `momentum_builtin` statische Adapter-Grenze); runtime natural-paper IDs via `format_runtime_signal_id()` (collision-safe UUID4, PR #3956/#3971); `correlation_ledger` SIGNAL writes mit `build_signal_ledger_payload()` + `campaign_id` aus `CDB_CAMPAIGN_ID` (PR #3961); insert conflicts exposed via `correlation_ledger_insert_conflicts_total` (PR #3971); Dockerfile `CDB_SOURCE_SHA` build marker (PR #3974); audit metadata: `config_snapshot` + `config_hash`; `SIGNAL_BOT_ID` (PR #2129); `source=stimulus_fixture` + `stimulus_run_id` deterministische replay/paper IDs; reserved metadata keys immutable; campaign overlays: parallel (#3918), diagnostic (#3964), diag-reverify (#3974), telemetry-pass (#3979) — nicht kanonischer Default, separate RUNTIME-GO |
...
| **Donchian Breakout Backtest Runner** | `services/validation/donchian_breakout_backtest_runner.py` | **AKTIV** (PR #3782) | Single-pass offline backtest für `donchian_breakout_v1` (Pack-A wave-1 Vergleichskandidat). Deterministische Shape-/Szenario-Replay-Fläche; `ranking_ready=false`; keine Promotion, kein Runtime-Pfad. |
| **Breakout Trend Filter Backtest Runner** | `services/validation/breakout_trend_filter_backtest_runner.py` | **AKTIV** (PR #3782) | Single-pass offline backtest für `breakout_trend_filter_v1` (Donchian + 5m-EMA-Trend-Gate). Deterministische Shape-/Szenario-Replay-Fläche; `ranking_ready=false`; keine Promotion, kein Runtime-Pfad. |
| **ARVP Replay CLI** | `services/validation/strategy_replay_runner.py` | **AKTIV** (PR #1808, PR #1859, PR #1891, PR #1944, PR #3782) | Operator-facing entry-point: `run_arvp_replay()` → `main()`. Strategy-aware dispatch für `primary_breakout_v1`, `donchian_breakout_v1`, `breakout_trend_filter_v1`, `range_mean_reversion_v1`, `momentum_capture_v1`. Validates fail-closed: `--dataset-source` (file\|db), `--speedup-profile`, scheduler metadata. Exponiert `--gate-trace-path` (PB1 only) und reicht diesen an den Backtest-Runner weiter. Scenario-group path schreibt `{scenario_id}_metrics.json` Zusammenfassungen. Source-aware baseline + scenario-group paths. Exit Codes 0/1/2 (success/validation/runtime). Config: ARVPReplayConfig mit dataset/strategy/adapter/scheduler params. |
...
| **Price Policy Evaluation CLI** | `tools/replay/evaluate_price_policies.py` | **AKTIV** (PR #3081) | Operator-facing CLI for `price_policy` evaluation against replay report. Produces price-policy-evaluation JSON + summary evidence; consumes `report.json` from a replay run. Used for #3079 price-policy gap analysis. Exit codes: 0 success / 1 usage error. |
| **Profitability Evidence Packet Assembler CLI** | `services/validation/profitability_evidence_packet_assembler.py` | **AKTIV** (PR #3404) | Offline, deterministic validation CLI (#3381). Validates explicit local input artifacts and assembles schema-validated `profitability_evidence_packet.v1` JSON + Markdown. Evidence/validation support only — no runtime/BLUE/RED, no DB mutation, no trading authorization, no candidate promotion, no capital allocation. LR NO-GO. Exit codes: 0 success / 1 usage / 2 validation error. |
| **Offline Strategy League Scorer CLI** | `services/validation/profitability_league_scorer.py` | **AKTIV** (PR #3685) | Offline, read-only, fail-closed Strategy League scorer v1 (#3682/#3684). Reads `profitability_evidence_packet.v1`, applies Formula v1 hard gates and weighted scoring, emits schema-validated `profitability_league_table_report.v1`. Sentinel on hard-gate failure; `PARK` is research hold only. Decision support only — no runtime/BLUE/RED, no DB mutation, no candidate promotion, no capital allocation. LR NO-GO. Exit codes: 0 success / 1 usage / 2 validation error. |
| **ARVP Candidate Evidence Assembler (offline library)** | `services/validation/arvp_candidate_evidence_assembler.py` | **AKTIV** (PR #4022) | Offline validation library (#4016): aggregates `arvp_strategy_metrics.v1` per candidate into deterministic `profitability_evidence_packet.v1` bundles. Operator CLI adapter: `tools/arvp_vacation/candidate_evidence_assembly.py`. Not a network service; no container/port; optional local artifact output only. `ranking_ready=false`; LR NO-GO. |
| **Governance League Table Report Assembler (offline library)** | `services/validation/profitability_league_table_report_assembler.py` | **AKTIV** (PR #4025) | Offline validation library (#4017): governance-safe `profitability_league_table_report.v1` from PEP bundles; ARVP rankability gates for official ranking. Uses `profitability_league_scorer.py` internally but is not interchangeable with the scorer CLI. Operator CLI adapter: `tools/arvp_vacation/league_table_report.py`. Not a network service; LR NO-GO. |
| **Correlation Ledger Attribution** | `core/replay/correlation_ledger_attribution.py` | **AKTIV** (PR #3961) | `CDB_CAMPAIGN_ID` resolution; `build_signal_ledger_payload()` / `build_decision_ledger_payload()`; `aggregate_lane_campaign_evidence()` with `blocks_by_reason`. Telemetry/evidence support only. |
| **Correlation Ledger Insert** | `core/replay/correlation_ledger_insert.py` | **AKTIV** (PR #3971) | Insert rowcount classification; false-zero risk evaluation; Prometheus counter parsing for `correlation_ledger_insert_conflicts_total`. |
...
**ARVP Vacation Evidence Pipeline (offline):** `tools/arvp_vacation/coordinator.py` (batch runner) → `tools/arvp_vacation/strategy_metric_extraction.py` (`arvp_strategy_metrics.v1`) → `services/validation/arvp_candidate_evidence_assembler.py` (`profitability_evidence_packet.v1`) → `services/validation/profitability_league_table_report_assembler.py` (`profitability_league_table_report.v1`). Libraries and operator CLIs only; not deployed BLUE/RED services; LR **NO-GO**.
...
| **Grafana** | cdb_grafana | grafana/grafana:13.1.3-ubuntu@sha256:ab9a06d495291c7ba210426b62e9056dba6046d0945f7e9af041f3ff29b4c7fe | 3000 | **AKTIV** | Dashboards |
...
| 2026-07-06 | PR #3782 Nachzug (#3783): `pack_a_breakout_common.py`, Donchian/Bo+Trend backtest runners + ARVP Replay CLI multi-strategy dispatch im Replay-Infrastruktur-Katalog ergänzt; offline validation only, keine Runtime-Erweiterung | Cursor |
| 2026-07-08 | PR #3915/#3918 Nachzug (#3916/#3919): Allocation `ALLOCATION_RULES_JSON` Donchian half-cap + parallel campaign overlays (`cdb_signal_pb1`/`cdb_signal_donchian`) in BLUE/RED-Katalogzeilen dokumentiert | Cursor |
| 2026-07-10 | PRs #3956/#3961/#3971/#3974 Nachzug (#3957/#3962/#3972/#3975/#3976): Signal/Risk telemetry chain, core/replay attribution+insert modules, `CDB_SOURCE_SHA` freshness, campaign overlay inventory | Cursor |
| 2026-07-13 | PR #4022 Nachzug (#4023): ARVP candidate evidence assembler + vacation CLI in Core-Libraries/Validation-Katalog | Cursor |
| 2026-07-13 | PR #4025 Nachzug (#4026): governance league table report assembler + vacation CLI; distinct from league scorer | Cursor |
| 2026-07-13 | Batch reconcile (#4012/#4020/#4023/#4026): #4011/#4018 navigation-only changes documented as NO_DRIFT_FALSE_POSITIVE | Cursor |
