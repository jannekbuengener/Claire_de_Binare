...
| **Pack-A Breakout Common** | `core/replay/pack_a_breakout_common.py` | Shared offline helpers for Pack-A wave-1 breakout variants: Donchian channel math, closed-bar-only 5m trend gate, candle validation. Frozen spec parameters per #3748; validation/shape-replay only, no runtime. | **AKTIV** (PR #3782) |
...
| **ARVP Replay CLI** | `services/validation/strategy_replay_runner.py` | Operator-facing entry-point (`run_arvp_replay()` → `main()`). Strategy-aware dispatch für `primary_breakout_v1`, `donchian_breakout_v1`, `breakout_trend_filter_v1`, `range_mean_reversion_v1`, `momentum_capture_v1`. Fail-closed: validates `--dataset-source` (file\|db), `--speedup-profile`, scheduler metadata. Exponiert `--gate-trace-path` (PB1 only) und reicht diesen an den Backtest-Runner weiter. Scenario-group path schreibt `{scenario_id}_metrics.json` Zusammenfassungen. Exit Codes 0/1/2. Source-aware baseline + scenario-group paths. | **AKTIV** (PR #1808, PR #1859, PR #1891, PR #1944, PR #3782) |
