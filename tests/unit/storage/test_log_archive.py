...
@pytest.mark.unit
def test_reparse_source_root_is_blocked(
...
@pytest.mark.unit
def test_reparse_candidate_is_blocked_before_metadata_or_hash(
    archive_roots: tuple[Path, Path], monkeypatch: pytest.MonkeyPatch
) -> None:
    source, _destination = archive_roots
    candidate = _write(source, "events_20260714.jsonl")

    monkeypatch.setattr(
        "tools.storage.log_archive._is_reparse_point",
        lambda path: path == candidate,
    )

    def fail_hash(_path: Path) -> str:
        raise AssertionError("candidate must be rejected before hashing")

    monkeypatch.setattr("tools.storage.log_archive._sha256", fail_hash)
...
@pytest.mark.unit
def test_source_equal_destination_is_blocked(
    ...
    monkeypatch.setattr(
        "tools.storage.log_archive.resolve_bulk_storage_path",
        lambda _subtree, environ=None: tmp_path,
    )

    with pytest.raises(LogArchiveError, match="SOURCE_DESTINATION_OVERLAP"):
        build_log_archive_plan(source, environ=ENV, as_of_utc=AS_OF)


@pytest.mark.unit
def test_destination_below_source_is_blocked(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(
        "tools.storage.log_archive.resolve_bulk_storage_path",
        lambda _subtree, environ=None: source / "bulk" / "logs",
    )
...
@pytest.mark.unit
def test_source_below_destination_is_blocked(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    destination = tmp_path / "archive" / "events"
...
@pytest.mark.unit
def test_apply_blocks_destination_reparse_before_copy_or_delete(
    archive_roots: tuple[Path, Path],
    tmp_path: Path,
    ...
    def fail_copy(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("copy must not run for a reparse destination")

    monkeypatch.setattr("tools.storage.log_archive.shutil.copyfile", fail_copy)
...
@pytest.mark.unit
def test_apply_evidence_path_cannot_alias_planned_source(
    archive_roots: tuple[Path, Path],
) -> None:
    source, destination = archive_roots
    file = _write(source, "events_20260714.jsonl", "original\n")
    plan = build_log_archive_plan(source, environ=ENV, as_of_utc=AS_OF)

    result = apply_log_archive_plan(plan, plan["plan_fingerprint"], file)

    assert result["result"] == "BLOCKED"
    assert result["failure_reason"] == "EVIDENCE_PATH_OVERLAP"
    assert file.read_text(encoding="utf-8") == "original\n"
    assert not (destination / file.name).exists()


@pytest.mark.unit
def test_apply_evidence_path_inside_source_is_blocked(
    archive_roots: tuple[Path, Path],
) -> None:
    source, destination = archive_roots
    file = _write(source, "events_20260714.jsonl", "original\n")
    evidence = source / "journal" / "result.json"
    plan = build_log_archive_plan(source, environ=ENV, as_of_utc=AS_OF)
...
@pytest.mark.unit
def test_apply_evidence_path_inside_destination_is_blocked(
    archive_roots: tuple[Path, Path],
) -> None:
    source, destination = archive_roots
    file = _write(source, "events_20260714.jsonl", "original\n")
    evidence = destination / "evidence" / "result.json"
    plan = build_log_archive_plan(source, environ=ENV, as_of_utc=AS_OF)
...
@pytest.mark.unit
def test_plan_hold_remains_binding_after_destination_is_created(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = tmp_path / "source"
    destination = tmp_path / "bulk" / "logs" / "events"
...
@pytest.mark.unit
def test_hold_entry_blocks_apply_and_is_preserved_in_evidence(
    archive_roots: tuple[Path, Path], tmp_path: Path
) -> None:
    source, destination = archive_roots
    file = _write(source, "events_20260714.jsonl", "source\n")
    _write(destination, file.name, "different\n")
    plan = build_log_archive_plan(source, environ=ENV, as_of_utc=AS_OF)
...
@pytest.mark.unit
def test_mixed_candidate_and_hold_performs_zero_partial_apply(
    archive_roots: tuple[Path, Path], tmp_path: Path
) -> None:
    source, destination = archive_roots
    held_source = _write(source, "events_20260713.jsonl", "held\n")
    candidate = _write(source, "events_20260714.jsonl", "candidate\n")
    _write(destination, held_source.name, "collision\n")
    plan = build_log_archive_plan(source, environ=ENV, as_of_utc=AS_OF)
    ...
    assert held_source.exists()
    assert candidate.exists()
    assert not (destination / candidate.name).exists()
