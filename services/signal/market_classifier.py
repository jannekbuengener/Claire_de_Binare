...
class MarketClassifier:
    ...
    def classify_current_market(self, lookback_period: str = 'medium') -> MarketMetrics:
        ...
        # Get lookback period
        periods = self.lookback_periods.get(lookback_period, self.lookback_periods['medium'])
        ...
        self.logger.debug(f"Market classified as {phase.value} (confidence: {confidence:.2f})")
    ...
    def _classify_phase(self, trend_strength: float, trend_direction: int, 
                       volatility_score: float, momentum: float) -> Tuple[MarketPhase, float]:
        """
        ...
        if trend_strength > 0.6:
            ...
            momentum_alignment = (
                (trend_direction > 0 and momentum > 0) or
                (trend_direction < 0 and momentum < 0) or
                (trend_direction == 0 and abs(momentum) < 0.2)
            )
    ...
    def get_market_summary(self, periods: List[str] = None) -> Dict[str, Any]:
        ...
        phases = [summary['timeframes'][p]['phase'] for p in periods 
        ...
        summary['overall_assessment'] = {
            ...
            'phase_agreement': max(phase_counts.values()) / len(phases) if phases else 0,
            'avg_confidence': np.mean([summary['timeframes'][p]['confidence'] 
                                     for p in periods if p in summary['timeframes']])
        }
    ...
    def get_phase_distribution(self, hours_back: int = 24) -> Dict[str, float]:
        ...
        recent_classifications = [
            m for m in self.classification_history 
    ...
    def should_trade_in_current_conditions(self, 
                                         min_confidence: float = 0.6,
                                         avoid_volatile_markets: bool = True) -> Dict[str, Any]:
        """
        ...
        if current_metrics.confidence < min_confidence:
            recommendation['reason'] = f"Low confidence ({current_metrics.confidence:.2f} < {min_confidence})"
        ...
        if current_metrics.phase in [MarketPhase.TRENDING_UP, MarketPhase.TRENDING_DOWN]:
            recommendation['risk_level'] = 'low' if current_metrics.trend_strength > 0.7 else 'medium'
