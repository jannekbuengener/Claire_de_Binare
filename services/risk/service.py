...
def _phase9_enrich_evidence(
    ...
    # Set input_hash in evidence (canonical field in evidence dict). Note that
    # _emit_risk_event will also set input_snapshot_hash as a persisted alias
    # for backwards compatibility at line 604, so both keys will be present
    # in the final persisted event.
    ...
    # NOTE: inputs may contain None values (e.g., regime_id, return_1m) which
    # accurately reflect the inputs used for the decision. JSON serialization
    # of None is deterministic and matches the input_hash computation which
    # includes these same fields via DECISION_HASH_FIELDS.
