...
class LiveTradingGate:
    ...
    def _validate_test_results(self, test_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not validation_result.get('overall_pass', False):
            ...
            if risk_level == 'medium':
                ...
                validation['reason'] = 'Limited authorization due to medium risk assessment'
            else:
                validation['valid'] = False
    ...
    def _determine_authorization_level(self, validation_result: Dict[str, Any]) -> AuthorizationLevel:
        """Determine appropriate authorization level"""
        if not validation_result.get('valid', False):
            return AuthorizationLevel.DENIED
        
        return validation_result.get('authorization_level', AuthorizationLevel.PAPER_ONLY)
    
    def _create_authorization_response(self, 
        ...
        if test_results:
            response['test_summary'] = {
                ...
                'validation_passed': test_results.get('validation_result', {}).get('overall_pass', False)
            }
