...
class EmojiAnalyzer:
    ...
    def scan_file(self, file_path: Path) -> List[EmojiDetection]:
        ...
        try:
            ...
            for emoji_info in emoji_lib_results:
                ...
                already_found = any(
                    d.line_number == content[:emoji_info['match_start']].count('\n') + 1 
                    for d in detections
                )
    ...
    def auto_fix_emojis(self, backup: bool = True) -> None:
        ...
        for detection in self.detections:
            if not detection.is_whitelisted and detection.severity in ['error', 'warning']:
...
def export_results(self, format: str = "json") -> None:
        ...
        if format == "json":
            ...
            with open("emoji-report.md", "w", encoding="utf-8") as f:
                ...
                f.write(f"**Whitelisted:** {report['summary']['whitelisted_emojis']}\n\n")
...
def main():
    ...
    parser.add_argument('--config', default='.github/emoji-config.yaml', help='Config file path')
