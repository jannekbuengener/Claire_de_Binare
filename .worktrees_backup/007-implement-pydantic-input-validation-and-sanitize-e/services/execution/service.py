...
try:
    ...
    from .validation import validate_order_payload, OrderValidationError
    from .error_handling import sanitize_error, sanitize_database_error, ERROR_SERVICE_UNAVAILABLE
except ImportError:
    import config
    from models import Order, ExecutionResult, OrderStatus
    from mock_executor import MockExecutor
...
def process_order(order_data: dict):
    """Process incoming order with Pydantic validation.

    This function validates incoming order payloads before processing to address
    penetration test finding M-01 (CVSS 5.3): Order payloads lack comprehensive
    validation.

    Args:
        order_data: Dictionary containing order data from Redis pub/sub

    Returns:
        ExecutionResult on success, or None if order processing fails
    """
    try:
        # Filter out non-order events before validation
        ...
        # Validate payload using Pydantic validation (M-01 fix)
        try:
            order = validate_order_payload(order_data)
        except OrderValidationError as validation_err:
            # Log validation details (not exposed to clients)
            logger.warning(
                "Order validation failed: %s - Details: %s",
                validation_err.message,
                validation_err.details,
            )
            # Create REJECTED result for invalid payloads
            validation_id = generate_uuid_hex(
                name=f"validation:{order_data.get('symbol', 'UNKNOWN')}:{utcnow().isoformat()}"
            )
            result = ExecutionResult(
                order_id=f"VALIDATION_{validation_id}",
                symbol=order_data.get("symbol", "UNKNOWN"),
                side=order_data.get("side", "UNKNOWN"),
                quantity=float(order_data.get("quantity", 0)) if order_data.get("quantity") else 0.0,
                filled_quantity=0.0,
                status=OrderStatus.REJECTED.value,
                price=None,
                client_id=order_data.get("client_id"),
                error_message="Order validation failed",  # Safe message, details logged only
...
def orders():
    ...
    This endpoint returns sanitized error responses to address penetration test
    finding L-01: The /orders endpoint returns raw exception messages exposing
    internal details.
    """
    if not db:
        # Database not initialized - use service unavailable response
        logger.error("Orders endpoint called but database not initialized")
        return jsonify({"error": ERROR_SERVICE_UNAVAILABLE, "code": "SERVICE_UNAVAILABLE"}), 503

    try:
        recent_orders = db.get_recent_orders(limit=20)
        return jsonify({"count": len(recent_orders), "orders": recent_orders}), 200
    except Exception as e:
        # Use sanitized error response (L-01 fix) - full error logged internally
        error_response = sanitize_database_error(e)
