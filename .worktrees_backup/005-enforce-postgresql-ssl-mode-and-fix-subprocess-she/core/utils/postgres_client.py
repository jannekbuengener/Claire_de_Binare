...
Provides a centralized way to create PostgreSQL connections with SSL enforcement.
All services should use this factory instead of direct psycopg2 calls.

Security Note:
    The default sslmode is 'require' to enforce encrypted connections and prevent
    man-in-the-middle (MITM) attacks. Using 'prefer' or weaker modes allows attackers
    to force TLS downgrade, exposing database credentials and sensitive trading data.
    Only override with weaker modes for local development with explicit understanding
...
Environment Variables:
    ...
    POSTGRES_SSLMODE: SSL mode (default: require)
        ...
        - require: Require SSL, no verification (default)
...
def get_postgres_dsn(
    ...
    sslmode = sslmode or os.getenv("POSTGRES_SSLMODE", "require")
